    ...
    const StatRow = ({ stat: { value, label, attr } }) => {
      return (
        <div className={statRowStyle}>
          <dt><abbr title={attr}>{label}</abbr></dt>
          <dd className={css`margin:0;font-size:1.2em;`}>{value}</dd>
          <dd className={css`margin:0;`}>{statMod(value)}</dd>
        </div>
      )
    }

    return (
      <dl className={css`display:flex;width:300px;justify-content:center;`}>
        {data.map((attr) =>
